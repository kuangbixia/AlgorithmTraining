# C++
## [C++基础](http://c.biancheng.net/cplus/)
#### 琐碎
##### static关键字
- 在函数内声明，也可以在函数外共用。
- 作为全局变量。
...
#### C++存储类
##### static ▲
1. 修饰局部变量时，仅在第一次被调用时初始化，第二次调用不会再初始化。可以在不同的函数调用之间保持局部变量的值，即不需要再每次进入和离开作用域时重复进行创建和销毁
2. 修饰全局变量，作用域为**声明它的文件**，只可以被当前文件访问；而不加static的全局变量，可以被其他文件访问
...
- **new vs malloc()函数 √**
    - new
        - 返回指定类型的指针
        - 用delete关键字释放内存
    - malloc()：是库函数，包含在头文件"malloc.h"中，用于申请一块连续的指定大小的内存空间
        - 返回void*，需要强制类型转换成对应的指针类型
        - 用free()函数释放内存
        - 可以通过remalloc()函数重新分配内存
##### delete
- 用来释放指针指向的内存
